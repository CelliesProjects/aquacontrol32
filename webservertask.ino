  ...
  **********************************************************************************************/
  ...
  [](AsyncWebServerRequest * request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
    ...
    if (!index) {
      ...
      if (request->hasHeader(CONTENT_LENGTH) && request->header(CONTENT_LENGTH).toInt() > FFat.freeBytes()) {
        char * buffer[80];
        snprintf((char*)buffer, sizeof(buffer), "No space for %s bytes. Max size is %i bytes.", request->header(CONTENT_LENGTH), FFat.freeBytes());
        AsyncWebServerResponse *response = request->beginResponse(501, HEADER_HTML, (char*)buffer);
        request->send(response);
      ...
      ESP_LOGI(TAG, "File transfer-> %s", filename.c_str());
    }
    ...
    if (final) {
      ...
      if (filename.equals(defaultTimerFile))
        defaultTimersLoaded();
      ESP_LOGI(TAG, "Upload %iBytes in %.2fs which is %.2ikB/s.", index + len, (millis() - startTimer) / 1000.0, index / (millis() - startTimer));  // insanely inaccurate
    }
  });
...
static inline __attribute__((always_inline)) int checkChannelNumber(const AsyncWebServerRequest * request) {

  if (!request->hasArg("channel") || !(request->arg("channel").toInt() < NUMBER_OF_CHANNELS))
    return INVALID_CHANNEL;
  return request->arg("channel").toInt();
}
